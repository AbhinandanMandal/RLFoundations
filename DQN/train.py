...
if __name__ == '__main__':
    ...
    agent_vanilla_dqn.train(num_episodes=num_episodes,
                            batch_size=batch_size, min_buffer_size=min_buffer_size)
    plot_reward(reward_list=agent_vanilla_dqn.return_list, title="Vanilla DQN")

    agent_double_dqn.train(num_episodes=num_episodes,
                           batch_size=batch_size, min_buffer_size=min_buffer_size)
    plot_reward(reward_list=agent_double_dqn.return_list, title="Double DQN")

    agent_dueling_dqn.train(num_episodes=num_episodes,
                            batch_size=batch_size, min_buffer_size=min_buffer_size)
