...
class VAnet(nn.Module):
    ...
    def forward(self, x):
        ...
        A = self.fc_A(x)
        V = self.fc_V(x)
        # take mean instead of max here to make training more stable
        Q = V + A - A.mean(dim=1, keepdim=True).view(-1, 1) # For stability in Duelling DQN
        return Q
...
#  TODO: Can be a thing to consider
"""
...
class DQN:
        def __init__(self, env, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon, target_update, buffer_size,
            ...
            if self.dqn_type == "duelingDQN":
                ...
                self.q_net = Qnet(state_dim, hidden_dim, action_dim).to(
                    device)  # the training q network used to choose
                # action
                self.target_q_net = Qnet(state_dim, hidden_dim, action_dim).to(
                    device)  # the target q network used to
        ...
        def update(self, states, actions, rewards, next_states, dones):
            ...
            rewards = torch.tensor(
                rewards, dtype=torch.float32).view(-1, 1).to(self.device)
            ...
            for i in range(num_episodes):
                ...
                while not done and not truncated:
                    ...
                    if self.replay_buffer.size() > min_buffer_size:
                        batch_states, batch_actions, batch_rewards, batch_next_states, batch_done = self.replay_buffer.sample(
                            batch_size)
                        self.update(batch_states, batch_actions,
                                    batch_rewards, batch_next_states, batch_done)
