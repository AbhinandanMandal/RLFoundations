...
from DynamicProgramming.value_iteration import ValueIteration
from DynamicProgramming.policy_iteration import PolicyIteration
from environment.clifwalk_env import ClifWalkingWithPEnv
...
if __name__ == "__main__":
    ...
    cliff_walk_env = ClifWalkingWithPEnv()
    action_meaning_cw = ['^', 'v', '<', '>']
    theta_cw = 1e-6
    gamma_cw = 0.9
    n_actions_cw = 4
    vi_cw = ValueIteration(env=cliff_walk_env, theta=theta_cw,
                           gamma=gamma_cw, n_actions=n_actions_cw)
    vi_cw.value_iteration()
    print("Cliff Walking Value Iteration Result:")
    print_agent(vi_cw, action_meaning_cw, list(range(37, 47)), [47])
    print("----------------")
    pi_cw = PolicyIteration(
        env=cliff_walk_env, theta=theta_cw, gamma=gamma_cw, n_actions=n_actions_cw)
    ...
    # rendered as textual mode instead of graphical
    ...
    theta_fl = 1e-5
    gamma_fl = 0.9
    vi_fl = ValueIteration(env=frozen_lake_env, theta=theta_fl,
                           gamma=gamma_fl, n_actions=n_actions_fl)
    vi_fl.value_iteration()
    action_meaning_fl = ['<', 'v', '>', '^']
    print("Frozen Lake Value Iteration Result:")
    print_agent(vi_fl, action_meaning_fl, [5, 7, 11, 12], [15])
    print("----------------")
    pi_fl = PolicyIteration(
        env=frozen_lake_env, theta=theta_fl, gamma=gamma_fl, n_actions=n_actions_fl)
